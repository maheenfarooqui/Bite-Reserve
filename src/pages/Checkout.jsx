...
const Checkout = () => {
  ...
  const { user } = useAuth(); // ✅ Context se user le liya
  ...
  const handleSubmit = async (e) => {
    ...
    const orderDetails = {
      userId: user?.uid || "Guest", // ✅ Sahi ID save hogi
      customerEmail: user?.email || "No Email",
      customerName: formData.name,
      phone: formData.phone,
      address: formData.address,
      items: cart,
      total: totalPrice, // ✅ totalAmount se 'total' kar diya
      status: "Pending", // ✅ Admin panel ke liye status
      createdAt: new Date(),
    };
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <input 
                ...
                className="w-full p-4 rounded-2xl border border-gray-100 bg-gray-50 focus:ring-2 focus:ring-primary outline-none transition-all"
            ...
            <button 
              ...
              className="w-full bg-primary text-white py-5 rounded-2xl font-black text-lg hover:bg-orange-600 transition-all shadow-xl shadow-primary/20 disabled:bg-gray-300"
            >
